...
//----------------------------------- Hauptprogramm --------------------------------------------------------------------
char deviceleft[100]= "enx04421a3c65e8";  // 2500 MBit USB
char deviceright[100]= "enx000acd400b7b";  // 2500 MBit USB
...
// Open A Raw Socket Deviceright -------------------------------------------------------------
  ...
  long double rrate= 30000;   // Bytes pro tick (1/100 Sekunde)  300 kB/s
  long double lrate= 30000;
  long double bucketsizeleft= lrate*100;
  long double bucketsizeright= rrate*100;
  long double bucketleft= bucketsizeleft;
  long double bucketright= bucketsizeright;
  long double resultleft= 0;
  long double resultright= 0;
  clock_t buckettime= times (0);
...
//      printf ("Packet %10ld  length: %5ld idx: %2d %20llx %10lld %10lld   not from cable\n", packetcounter, result, sockaddrboth.sll_ifindex, llmac (transferpuffer + 6), emplisteleft.anz, emplisteright.anz);
    ...
    // Buckets ausgeben
      for (long double lauf= 0; lauf < bucketsizeleft; lauf+= 20000)
        if (lauf < bucketleft)
          printf ("l");
          else
          printf ("-");
      printf ("\n");
      for (long double lauf= 0; lauf < bucketsizeright; lauf+= 20000)
        if (lauf < bucketright)
          printf ("r");
          else
          printf ("-");
    ...
    if ((sockaddrboth.sll_ifindex == indexleft) && (emplisteright.neu (transferpuffer + 6)))
      ...
      bucketleft+= (times (0) - buckettime) * lrate - resultleft;
      buckettime= times (0);
      if (bucketleft > bucketsizeleft)
        bucketleft= bucketsizeleft;
      if (bucketleft < 0)
        {
        bucketleft+= resultleft;
        printf ("------------------------------------ leftunderrun\n");
        continue;
        }
      
      if (drand48 ()*bucketsizeleft*0.5 <= bucketleft)
        {
        nowr= times (NULL);
    ...
    if ((sockaddrboth.sll_ifindex == indexright) && (emplisteleft.neu (transferpuffer + 6)))
      ...
      bucketright+= (times (0) - buckettime) * rrate - resultright;
      buckettime= times (0);
      if (bucketright > bucketsizeright)
        bucketright= bucketsizeright;
      if (bucketright < 0)
        {
        bucketright+= resultright;
        printf ("--------------------------------- rightunderrun\n");
        continue;
        }
      
      if (drand48 ()*bucketsizeright*0.5 <= bucketright)
        {
        nowl= times (NULL);
