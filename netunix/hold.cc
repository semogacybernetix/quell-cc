...
void empfangeudp ()
  ...
  printf ("%s", puffer);
  printf ("\n");
  FILE* datei= fopen ("iapetus.txt", "w");
  fprintf (datei, "%s", ipaddr);
  fprintf (datei, "\n");
  fclose (datei);
  int ressocket= socket (AF_INET, SOCK_DGRAM, 0);
  printf ("ressocket: %4d", ressocket);
  sockaddr_in resaddr;
  bzero (&resaddr, sizeof (resaddr));
  resaddr.sin_family= AF_INET;
  resaddr.sin_port= htons (7593);
  resaddr.sin_addr.s_addr= fromaddr.sin_addr.s_addr;
  for (unsigned long lticks= 0; lticks < 720; lticks++)
    {
    sendto (ressocket, "respond\n", 10, 0, (sockaddr*) &resaddr, sizeof (resaddr));
  //  printf ("bla\n");
    sleep (10);
    }
  }
