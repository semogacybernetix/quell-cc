...
void cbmpdatei::getpixel (integer px, integer py, integer& pr, integer& pg, integer& pb)
  {
  if ((px < 0) || (px >= xanz) || (py < 0) || (py >= yanz))          // Bildschirminhalt außerhalb der Abmessungen ist schwarz
    {
    pr= 0;
    pb= 0;
    pg= 0;
...
void cbmpdatei::putpixel (integer px, integer py, integer pr, integer pg, integer pb)
  {
  if ((px < 0) || (px >= xanz) || (py < 0) || (py >= yanz))          // Bildschirminhalt außerhalb der Abmessungen ist schwarz
    return;
  integer pos= py*zeile + px*3;
