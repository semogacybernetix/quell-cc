...
cpsdatei::cpsdatei (const char* pname, integer px, integer py)
  : clscreen8 (px, py), schreiben (true), zeile (3*px), bsize (zeile*py)
  {
  name= pname;
  vbild= new unsigned char[bsize];
  }
...
void cpsdatei::putpixel (integer px, integer py, integer pr, integer pg, integer pb)
  {
  if ((px >= xanz) || (py >= yanz))
    return;
...
void cpsdatei::getpixel (integer px, integer py, integer &pr, integer &pg, integer &pb)
  {
  if ((px >= xanz) || (py >= yanz))
    {
    pr= pg= pb= 0;
...
void cpsdatei::flush ()
  ...
  real psx= powr (2, real (-2.25))*1000*mm - 10*mm;
  real psy= powr (2, real (-1.75))*1000*mm - 10*mm;
