...
void quartischdiffpuintr (real aq, real bq, real cq, real dq, cschnittpunkte& psp)
  {
  real aqq, pq, qq, rq, pqq, rq4, pk, qk, xl, ytk, yk, l, pq6, z, uq, u, v, b1, b2, aq4, a1, a2, D12, D34, x1, x2, x3, x4;
...
void quartischdiffpvintr (real aq, real bq, real cq, real dq, cschnittpunkte& psp)
  {
  real aqq, pq, qq, rq, pqq, rq4, pk, qk, xl, ytk, yk, l, z, v, u, uq, b1, b2, aq4, a1, a2, D12, D34, x1, x2, x3, x4;
...
/*
    ...
    yk= (ytk + pk/ytk)/2;                                         // ytk = 0 nur bei qk= pk= xl= 0
//*/
//*
    if (qk > 0)
...
void quartischdiffpuvintr (real aq, real bq, real cq, real dq, cschnittpunkte& psp)
  {
  real aqq, pq, qq, rq, pqq, rq4, pk, qk, xl, ytk, yk, l, pq6, zk, uq, vq, u, v, bed, b1, b2, aq4, a1, a2, D12, D34, x1, x2, x3, x4;
...
/*
  u= sqrtr (uq);
  if (uq >= vq)                   // v-Fehler Torusmitte weg
    v= qq/u/4;
    else                          // Torusfehler außen weg, Sprühen bei (Entfernung und Drehung)
    v= sqrtr (vq);
//*/

/*
  v= sqrtr (vq);
  if (vq > uq)                    // v-Fehler Torusröhren weg, Feuer bei (Entfernung oder Drehung)
    u= qq/v/4;
    else
    u= sqrtr (uq);
//*/
...
//*
  u= sqrtr (uq);
  if ((xl < 0) && (u > 1e-2))     // v-Fehler Torusmitte weg, komplett sauber außerhalb,  1e-1 Fehler Mitte, 1e-5 Fehlerpixel außerhalb, 1e-2, 1e-3 Optimum
    v= qq/u/4;
    else
    v= sqrtr (vq);
//*/
...
/*
  v= sqrtr (vq);
  if ((xl >= 0) && (v > 1e-3))    // v-Fehler Torusröhren weg, Feuer bei (Entfernung und Drehung)
    u= qq/v/4;
    else
    u= sqrtr (uq);
//*/

  // Bedingung -2uv = qq  Gleichung wegen Rechenungenauigkeit nicht immer erfüllt
  bed= u*v*-2;
...
/*
  if ((xl < 0) && (u > 1e-2))                                     // volle Sahne, behebt den v-Fehler in der Mitte des Torusses
    v= qq/u/4;
    else
    u= qq/v/4;
*/
  ...
  if (xl < 0)
    {
    l= sqrtr (pk);
    real yk2= cosr (acosr (qk/pk/l)/3 - PI2d)*l;
    real yk3= cosr (acosr (qk/pk/l)/3 + PI2d)*l;
  printtext ("yk2: ");
  printreal (yk2);
  printtext ("\n");
  printtext ("yk3: ");
...
void quartischmalin (real aq, real bq, real cq, real dq, cschnittpunkte& psp)
  {
  real ak, bk, ck, ak2, pk, qk, pk3, qk2, ak3, ztk, zk, Dq, b1, b2, a1h, a2h, a1q, a2q, D12, D34, x1, x2, x3, x4;
...
/*
    real vxl= sqrtr (qk2 - pk3);                                     // Cardano-Berechnung langsamer, weil 2 Kubikwurzeln berechnet werden müssen
    zk= -cbrtr (qk + vxl) - cbrtr (qk - vxl) - ak3;                  // außerdem zusätzliche Stern-Artefakte beim Torus
//*/
//*
    if (qk >= 0)                                                     // Fallunterscheidung notwendig, sonst zusätzliche Stern-Artefakte beim Torus
    ...
    zk= ztk + pk/ztk - ak3;                                         // ytk = 0 nur bei qk= pk= xl= 0
//*/
/*
...
//*/
    ...
    zk= cosr (acosr (-qk/sqrtr (pk3))/3)*sqrtr (pk)*-2 - ak3;
  ...
  if (absr (Dq) > quant)
    ...
    b1= (zk + sqrtr (Dq))/2;
    b2= (zk - sqrtr (Dq))/2;
    a1h= (aq*b1 - cq)/(b1 - b2)/-2;
    a2h= (aq*b2 - cq)/(b2 - b1)/-2;
    }
    else
    {
    b1= zk/2;
  ...
  return;
  //if (finite (zk)) return;
  //if (finite (zk) && finite (b1) && finite (b2) && finite (a1h) && finite (a2h)) return;
