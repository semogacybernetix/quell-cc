...
//  printbasis3mat (divbasis, 0, 0);
//  printvektor3ord (vlin, 0);
//  getchar ();
...
// ----------------- obj print -------------------------------
  ...
  cvektor3 efk= cluster.ermittleefkdreibein (cluster.kliste->v[0]);
//  cvektor3 efk= cluster.ermittleefkdreibein (cvektor3 (el1.i, el1.j, el1.ij));
  cvektor3 vecke= cluster.fliste->v[(unsigned long long) (efk.x)];
  cvektor3 vflaeche= cluster.eliste->v[(unsigned long long) (efk.y)];
  cvektor3 vkante= cluster.kliste->v[(unsigned long long) (efk.z)];
  printtextobj (objdatei, "# Eckenvektor:   ");
  ...
  printtextobj (objdatei, "\n");
//  printbasis3mat (divbasis, 0, 0);
//  printvektor3ord (efkein, 0);
//  getchar ();
  vein= divbasis*efkein;
//  vein= efkein.x*vecke + efkein.y*vflaeche + efkein.z*vkante;
  printtextobj (objdatei, "# Körpervektor:          ");
  printvektor3perfobj (objdatei, vein, 10);
  printtextobj (objdatei, "\n");
  nvein= normiere (vein);
