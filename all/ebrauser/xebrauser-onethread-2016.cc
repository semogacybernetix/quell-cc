...
/*
2015.10.01
Bewertungen unter 3 eingeführt, setstandorte muss noch ohne Fehlerabfrage gemacht werden, Zuggenerator verbessern mit -1, 0, +1 geht nicht
Pattstellungen können gespeichert werden, illegale Stellungen bool bei setstandorte weg Rekord: 2:58:36 statt 3:36 verschnellert
Pattproblem und Endstellungen mit mehreren Figuren, Zugenerator liften
Matt, Patt, an eine bestimmte Position drängen Ecke, Rand
*/
...
struct cvier
  ...
  const signed long bewmax= (1 << 15) - 1;   // bewmax + 1: ungültige Stellungen
  const signed long bewmin= 3;
  const signed long bewpatt= 2;
  const signed long bewillegal= 1;
  const signed long bewun= 0;

  signed long  dieder[8][65];
  signed long  gangart[5];       // bitcodiert: König, Springer, Läufer, Turm, 0 = leeres Feld
  signed long  figurfarbe[5];    // 0: leer, 1: weiss, -1: schwarz
  ...
  void initfiguren (signed long pfig1, signed long pfig2, signed long pfig3, signed long pfig4)
    {
    figurfarbe[0]= 0;
    figurfarbe[1]= sgn (pfig1);
    figurfarbe[2]= sgn (pfig2);
    figurfarbe[3]= sgn (pfig3);
    figurfarbe[4]= sgn (pfig4);
    gangart[0]= 0;
    gangart[1]= abs (pfig1);
    gangart[2]= abs (pfig2);
    gangart[3]= abs (pfig3);
    ...
    figurnrfb[0]= 0;
    figurnrfb[1]= 1*sgn (pfig1);
    figurnrfb[2]= 2*sgn (pfig2);
    figurnrfb[3]= 3*sgn (pfig3);
    figurnrfb[4]= 4*sgn (pfig4);
    }
  ...
  // Brett für den Zuggenerator zum Zügetesten setzen
  void setgbrett (signed long* pstandort, signed long* pbrett)
    {
    memset (&*pbrett, 0, 64*sizeof (*pbrett));
    pbrett[pstandort[1]]= figurfarbe[1];
    pbrett[pstandort[2]]= figurfarbe[2];
    pbrett[pstandort[3]]= figurfarbe[3];
    pbrett[pstandort[4]]= figurfarbe[4];
    }

  // Standorte -> Brett
  void setzbrett (signed long* pstandort, signed long* pbrett)
    {
    memset (&*pbrett, 0, 64*sizeof (*pbrett));
...
void testekoenig (signed long pfeldnr, const signed long* pbrett, signed long* phinfeld)
  ...
  if (linksok (pfeldnr) && (farbe != pbrett[pfeldnr - 1]))
      {
      phinfeld[phinfeld[64]]= pfeldnr - 1;
      phinfeld[64]++;
      }
  if (rechtsok (pfeldnr) && (farbe != pbrett[pfeldnr + 1]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 1;
    phinfeld[64]++;
    }
  ...
  if (obenok (pfeldnr) && linksok (pfeldnr) && (farbe != pbrett[pfeldnr + 7]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 7;
    phinfeld[64]++;
    }
  if (obenok (pfeldnr) && rechtsok (pfeldnr) && (farbe != pbrett[pfeldnr + 9]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 9;
    phinfeld[64]++;
    }
  if (untenok (pfeldnr) && linksok (pfeldnr) && (farbe != pbrett[pfeldnr - 9]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 9;
    phinfeld[64]++;
    }
  if (untenok (pfeldnr) && rechtsok (pfeldnr) && (farbe != pbrett[pfeldnr - 7]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 7;
    phinfeld[64]++;
    }
...
void testespringer (signed long pfeldnr, const signed long* pbrett, signed long* phinfeld)
  {
  signed long farbe= pbrett[pfeldnr];
  if (drechtsok (pfeldnr) && obenok (pfeldnr) && (farbe != pbrett[pfeldnr + 10]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 10;
    phinfeld[64]++;
    }
  if (rechtsok (pfeldnr) && dobenok (pfeldnr) && (farbe != pbrett[pfeldnr + 17]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 17;
    phinfeld[64]++;
    }
  if (linksok (pfeldnr) && dobenok (pfeldnr) && (farbe != pbrett[pfeldnr + 15]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 15;
    phinfeld[64]++;
    }
  if (dlinksok (pfeldnr) && obenok (pfeldnr) && (farbe != pbrett[pfeldnr + 6]))
    {
    phinfeld[phinfeld[64]]= pfeldnr + 6;
    phinfeld[64]++;
    }
  if (dlinksok (pfeldnr) && untenok (pfeldnr) && (farbe != pbrett[pfeldnr - 10]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 10;
    phinfeld[64]++;
    }
  if (linksok (pfeldnr) && duntenok (pfeldnr) && (farbe != pbrett[pfeldnr - 17]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 17;
    phinfeld[64]++;
    }
  if (rechtsok (pfeldnr) && duntenok (pfeldnr) && (farbe != pbrett[pfeldnr - 15]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 15;
    phinfeld[64]++;
    }
  if (drechtsok (pfeldnr) && untenok (pfeldnr) && (farbe != pbrett[pfeldnr - 6]))
    {
    phinfeld[phinfeld[64]]= pfeldnr - 6;
    phinfeld[64]++;
    }
...
void testeturm (signed long pfeldnr, const signed long* pbrett, signed long* phinfeld)
  ...
  feldlauf= pfeldnr;
  while (linksok (feldlauf) && (farbe != pbrett[feldlauf - 1]))
    {
    phinfeld[phinfeld[64]]= feldlauf - 1;
    phinfeld[64]++;
    if (pbrett[feldlauf - 1] != 0)
      break;
    feldlauf--;
    }
  feldlauf= pfeldnr;
  while (rechtsok (feldlauf) && (farbe != pbrett[feldlauf + 1]))
    {
    phinfeld[phinfeld[64]]= feldlauf + 1;
    phinfeld[64]++;
    if (pbrett[feldlauf + 1] != 0)
  ...
  feldlauf= pfeldnr;
  while (obenok (feldlauf) && (farbe != pbrett[feldlauf + 8]))
    {
    phinfeld[phinfeld[64]]= feldlauf + 8;
    phinfeld[64]++;
    if (pbrett[feldlauf + 8] != 0)
      break;
    feldlauf+= 8;
    }
  feldlauf= pfeldnr;
  while (untenok (feldlauf) && (farbe != pbrett[feldlauf - 8]))
    {
    phinfeld[phinfeld[64]]= feldlauf - 8;
    phinfeld[64]++;
    if (pbrett[feldlauf - 8] != 0)
...
void testelaeufer (signed long pfeldnr, const signed long* pbrett, signed long* phinfeld)
  ...
  signed long feldlauf;
  feldlauf= pfeldnr;
  while (linksok (feldlauf) && obenok (feldlauf) && (farbe != pbrett[feldlauf + 7]))
    {
    phinfeld[phinfeld[64]]= feldlauf + 7;
    phinfeld[64]++;
    if (pbrett[feldlauf + 7] != 0)
      break;
    feldlauf+= 7;
    }
  feldlauf= pfeldnr;
  while (rechtsok (feldlauf) && obenok (feldlauf) && (farbe != pbrett[feldlauf + 9]))
    {
    phinfeld[phinfeld[64]]= feldlauf + 9;
    phinfeld[64]++;
    if (pbrett[feldlauf + 9] != 0)
      break;
    feldlauf+= 9;
    }
  feldlauf= pfeldnr;
  while (linksok (feldlauf) && untenok (feldlauf) && (farbe != pbrett[feldlauf - 9]))
    {
    phinfeld[phinfeld[64]]= feldlauf - 9;
    phinfeld[64]++;
    if (pbrett[feldlauf - 9] != 0)
  ...
  feldlauf= pfeldnr;
  while (rechtsok (feldlauf) && untenok (feldlauf) && (farbe != pbrett[feldlauf - 7]))
    {
    phinfeld[phinfeld[64]]= feldlauf - 7;
    phinfeld[64]++;
    if (pbrett[feldlauf - 7] != 0)
...
void testefigur (signed long pfigurart, signed long pfeldnr, const signed long* pbrett, signed long* phinfeld)
  {
  phinfeld[64]= 0;
  if (pfigurart & 1)
...
void testepos (signed long pstellungsnr, signed long* phinpos)
  ...
  phinpos[256]= 0;
  // Hinfelder sammeln
  signed long gbrett[65];
  signed long zbrett[65];
  setgbrett (&*standort, &*gbrett);
  setzbrett (&*standort, &*zbrett);
  for (signed long figurlauf= 1; figurlauf < 5; figurlauf++)
    if ((standort[figurlauf] != 64) && (figurnrfb[figurlauf]*amzug > 0))
      {
      testefigur (gangart[figurlauf], standort[figurlauf], &*gbrett, &*hinfeld);
      // gefundene Züge ausführen und die entsprechenden Positionen sammeln
      for (signed long feldlauf= 0; feldlauf < hinfeld[64]; feldlauf++)
        {
        standort[figurlauf]= hinfeld[feldlauf];
        standort[abs (zbrett[hinfeld[feldlauf]])]= 64;
        amzug= -amzug;
...
signed long setendpos31 ()
  ...
  for (signed long poslauf=0; poslauf < endmax; poslauf++)
    ...
    if (endspiel[poslauf] == 0)
      ...
      // Gewinnstellung für weiss, eine von den weißen Figuren existiert und schwarzer König geschlagen ist und schwarz am Zug ist
      if (((standort[1] != 64) || (standort[2] != 64) || (standort[3] != 64)) && (standort[4] == 64) && (amzug == -1))
        {
        endspiel[poslauf]= bewmax;
        bewanz++;
        }
      // Gewinnstellung für schwarz, wenn schwarzer König existiert und beide weiße Figuren geschlagen sind und weiß am Zug ist
      if (((standort[1] == 64) && (standort[2] == 64) && (standort[3] == 64)) && (standort[4] != 64) && (amzug == 1))
        {
        endspiel[poslauf]= -bewmax;
        bewanz++;
        ...
        endspiel[poslauf] = bewillegal;   // ungültige Stellungen markieren
    }
  printf ("\n");
  return bewanz;
  }
...
signed long setecke31 ()
  ...
  for (signed long poslauf=0; poslauf < endmax; poslauf++)
    ...
    if (endspiel[poslauf] == 0)
      ...
      setstandorte (poslauf, &*standort, &amzug);
      // Gewinnstellung für weiss, eine von den weißen Figuren existiert und schwarzer König geschlagen ist und schwarz am Zug ist
      if (((standort[1] != 64) || (standort[2] != 64) || (standort[3] != 64)) && (standort[4] == 0) && (amzug == -1))
        {
        endspiel[poslauf]= bewmax;
        bewanz++;
        }
...
signed long setendpos21 ()
  ...
  for (signed long poslauf=0; poslauf < endmax; poslauf++)
    ...
    if (endspiel[poslauf] == 0)
      ...
      setstandorte (poslauf, &*standort, &amzug);
      // Gewinnstellung für weiss, eine von den weißen Figuren existiert und schwarzer König geschlagen ist und schwarz am Zug ist
      if (((standort[1] != 64) || (standort[2] != 64)) && (standort[3] == 64) && (amzug == -1))
        {
        endspiel[poslauf]= bewmax;
        bewanz++;
        }
      ...
      // ungültige Stellung, wenn 3 Figuren geschlagen sind
      if ((standort[1] == 64) && (standort[2] == 64) && (standort[3] == 64))
        {
        endspiel[poslauf]= bewillegal;
        bewanz++;
        }
...
signed long setendpos11 ()
  ...
  for (signed long poslauf=0; poslauf < endmax; poslauf++)
    ...
    if (endspiel[poslauf] == 0)
      ...
      // ungültige Stellung, wenn beide Könige geschlagen sind
      if ((standort[1] == 64) && (standort[2] == 64))
        {
        endspiel[poslauf]= bewillegal;
        bewanz++;
        }
...
signed long setendposecke11 ()
  ...
  for (signed long poslauf=0; poslauf < endmax; poslauf++)
    ...
    if (endspiel[poslauf] == 0)
      {
      setstandorte (poslauf, &*standort, &amzug);
      // Gewinnstellung für weiss, wenn weißer König existiert und schwarzer König in der Ecke ist und schwarz am Zug ist
      if ((standort[1] != 64) && (standort[2] == 0) && (amzug == -1))
        {
        endspiel[poslauf]= bewmax;
        bewanz++;
        }
      // Gewinnstellung für schwarz, wenn schwarzer König existiert und weißer König in der Ecke ist und weiß am Zug ist
      if ((standort[1] == 0) && (standort[2] != 64) && (amzug == 1))
        {
        endspiel[poslauf]= -bewmax;
        bewanz++;
        }
...
void analysiere ()
  {
  printf ("analysiere...\n");
  signed long illegalpos= setillegalpos ();
  printf ("%10ld illegale Positionen\n\n", illegalpos);
  signed long bewanz= setendpos21 ();
  signed long sum= 0;
  signed long ebene= 0;
    do
    {
...
// Anzeige, Visualisierung, Verwaltung der Endspieldatenbank
  ...
  void saveendspiel (const char* pname)
    ...
    memset (header, 0, 64);
    header[0]= 4;
    header[1]= figurfarbe[1] * gangart[1];
    header[2]= figurfarbe[2] * gangart[2];
    header[3]= figurfarbe[3] * gangart[3];
    header[4]= figurfarbe[4] * gangart[4];
    header[8]= maxposw;
    header[9]= maxposw >> 8;
  ...
  void brauseendspiel (signed long ppos)
    ...
    while (1)
      ...
      if (endspiel[ppos] == bewmax)
        printf ("Stellung: %10ld Gewinnstellung weiss\n", ppos);
      if (endspiel[ppos] == -bewmax)
        printf ("Stellung: %10ld Gewinnstellung schwarz s\n", ppos);
      if (endspiel[ppos] == bewmax - 1)
        printf ("Stellung: %10ld Schlagposition weiss\n", ppos);
      if (endspiel[ppos] == -(bewmax - 1))
        printf ("Stellung: %10ld Schlagposition schwarz\n", ppos);
      if (endspiel[ppos] == bewmax - 2)
        printf ("Stellung: %10ld Matt für weiss\n", ppos);
      if (endspiel[ppos] == -(bewmax - 2))
        printf ("Stellung: %10ld Matt für schwarz\n", ppos);
      if ((endspiel[ppos] >= bewmin) && (endspiel[ppos] < bewmax - 2))
          printf ("Stellung: %10ld Matt in %6.1f\n", ppos,  ((bewmax - 2) - abs (endspiel[ppos]))/2.);
      if ((endspiel[ppos] <= -bewmin) && (endspiel[ppos] > -(bewmax - 2)))
          printf ("Stellung: %10ld Matt in %6.1f\n", ppos, -((bewmax - 2) - abs (endspiel[ppos]))/2.);
      if (endspiel[ppos] == bewpatt)
        printf ("Stellung: %10ld Patt\n", ppos);
      if (abs (endspiel[ppos]) == bewillegal)
        printf ("Stellung: %10ld ungültige Stellung\n", ppos);
      if (endspiel[ppos] == bewun)
  ...
  void getextrempos2 ()
    ...
    for (signed long poslauf= 0; poslauf < endmax; poslauf++)
      ...
      if ((standort[4] != 64) || (standort[3] != 64))
        continue;
      if (abs (endspiel[poslauf]) >= bewmin)
        {
...
int main (int argc, char** argv)
  ...
  cvier steiner;
///*
  steiner.initfiguren (8, 2, -1, -2);
  steiner.analysiere ();
  steiner.getextrempos2 ();
