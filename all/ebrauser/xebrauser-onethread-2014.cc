...
// Anzeige, Visualisierung, Verwaltung der Endspieldatenbank
  ...
  void ladeendspiel (char* pname)
    {
    signed long long retf;
    ...
    signed char header[64];
    retf= fread (header, 64, 1, datei);
    retf= fread (endspiel, 1, endmax*sizeof (*endspiel), datei);
    fclose (datei);
    ...
    retf++;  // Variable benutzen, damit der Compiler nicht meckert, dass man sie nicht benutzt hat
    }
...
// längste Zugfolge bei geschlagener 4. Figur (um 3-Steiner Maximalposition zu bekommen)
  void getextrempos3 ()
    {
    signed long bestposw= 0;
    signed long bestposs= 0;
    signed long bestbeww= bewmax;
    signed long bestbews= -bewmax;
    signed long standort[65];
    signed long amzug;
...
int main (int argc, char** argv)
  ...
  argv+= 0;           // damit der Compiler nicht wegen einer unbenutzen Variable meckert
  cvier steiner;
  char* endname= argv[1];

  steiner.initfiguren (1, -1, 12, -4);
  steiner.analysiere ();
  steiner.getextrempos ();
  steiner.saveendspiel (endname);

  steiner.ladeendspiel (endname);
